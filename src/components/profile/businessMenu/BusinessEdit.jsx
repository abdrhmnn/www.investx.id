...
class BusinessEdit extends Component {
  ...
  componentDidMount() {
    ...
    this.apiProvince();
    this.checkall();
    this.getCompanyDetail(this.props.match.params.id);
  }

  checkall = () => {
    API.refCheckCompanyMe()
      .then((res) => {
        ...
        if (res.data.results.length !== 0) {
          ...
          if (
            ...
            API.getCompanyDetail(res.data.results[0].id62)
              .then((val) => {
                this.setState({
                  ...this.state,
                  ...val.data,
      ...
      .catch((err) => {
        ...
        Swal.fire({
          ...
          result.isConfirmed ? this.props.history.push('/') : null
        );
      });
  };

  getObjOpt = () => {
    API.refCompanyGeneral()
      .then((res) => {
        console.log(res);
        this.setState({
    ...
    API.getCompanyDetail(id)
      .then((res) => {
        ...
        this.setState({
          ...
          company_age: res.data.company_age,
          number_of_branches: res.data.number_of_branches,
          number_of_employees: res.data.number_of_employees,
          description: res.data.description,
          website_url: res.data.website_url,
          logo: res.data.logo,
          prospectus: res.data.prospectus,
        });
      })
      .catch((err) => console.log(err.response));
  };
  ...
  render() {
    ...
    const schemaObj = Yup.object({
      ...
      number_of_branches: Yup.number()
        .typeError('value have to be number')
        .required(),
    ...
    const locationInputForms = [
      {
        ...
        getData: null,
        clearData: ['regency', 'district', 'village'],
        prevId: null,
      },
      {
        key: 'regency',
        label: 'Kota/Kabupaten',
        data: this.state.regencyData,
        getData: (id) => this.apiRegency(id),
        clearData: ['district', 'village'],
        prevId: 'province',
      },
      {
        key: 'district',
        ...
        getData: (id) => this.apiDistrict(id),
        clearData: ['village'],
        prevId: 'regency',
      },
      {
        key: 'village',
        label: 'Kelurahan',
        data: this.state.villageData,
        getData: (id) => this.apiVillage(id),
        clearData: [],
        prevId: 'district',
      },
    ];
          ...
          <Formik
            ...
            onSubmit={(val) => {
              ...
              const body = {
                ...
                prospectus: val.prospectus?.url,
                logo: val.logo?.url,
              };
              this.setState({ loading: true });
              // API.postCompanyGeneral(body)
              //   .then((res) => {
              ...
              //       timer: 1500,
              //     }).then(() =>
              //       this.props.history.push('/startup-form-informasi-finansial')
              //     );
              //   })
              //   .catch((err) => {
              //     this.setState({ loading: false });
              ...
              //     console.log(err.response);
              //   });
              API.editCompany(this.props.match.params.id, body)
                .then(() => {
                  Swal.fire({
                    icon: 'success',
                    title: `Data ${this.state.pageName} berhasil di simpan`,
                ...
                <div className="row">
                    ...
                    return (
                      <div className="col-md-6" key={i}>
                        <InputSelect
                          ...
                          helperText={
                            touched[`${res.key}`] && errors[`${res.key}`]
                          }
                          error={
                            touched[`${res.key}`] && errors[`${res.key}`]
                              ? true
                          ...
                          onOpen={() =>
                            res.prevId !== null &&
                            res.getData !== null &&
                            values[res.prevId] !== null
                              ? res.getData(values[res.prevId].id)
                              : null
                          }
                        />
                      </div>
                  ...
                  <div className="col-md-12">
                    <Field
                      ...
                      // placeholder='Tempat Lahir *'
                      helperText={
                        touched.number_of_branches && errors.number_of_branches
                      }
                      error={
                        touched.number_of_branches && errors.number_of_branches
                          ? true
                  ...
                  <div className="col-md-12">
                    <Field
                      ...
                      error={
                        touched.website_url && errors.website_url ? true : false
                      }
                    />
                  </div>
                  ...
                  {/* <div className="col-md-12 startup-company-logo mb-4">
                    ...
                    <div className="file-frame">
                      <span>
                        {values.prospectus
                          ? values.prospectus.name
                          : 'Select File...'}
                      </span>
                      <InputFiles
                        accept="application/pdf"
                        onChange={(files) =>
                          this.handleFileUploadPdf(
                            files,
                            'prospectus',
                            setFieldValue
                          )
                    ...
                    <p className="info-file mb-0">
                      *File data <span>Pdf</span> dan tidak lebih dari{' '}
                      <span>5MB</span>
                    </p>
                  </div>
                  ...
                  <div className="col-md-12 startup-company-logo">
                    ...
                    <div className="file-frame">
                      <span>
                        {values.logo ? values.logo.name : 'Select File...'}
                      </span>
                      <InputFiles
                        onChange={(files) =>
                    ...
                    <p className="info-file">
                      *File data <span>Jpeg / PNG</span> dan tidak lebih dari{' '}
                      <span>5MB</span>{' '}
                    </p>
                  </div> */}
                  ...
                  <div className="col-md-12">
                    <Field
                      ...
                      error={
                        touched.prospectus && errors.prospectus ? true : false
                      }
                      InputProps={{
                        readOnly: true,
                        endAdornment: (
